...
function Order(props) {
  ...
  let formatTransaction = (trans) => {
    ...
    return {
      ...
      ship_date: checkShipdateNull
        ? ''
        : moment(trans.ship_date).format('DD/MM/YYYY'),
    };
  };
  ...
  useEffect(() => {
    let getData = async () => {
      ...
      let getTransactions = await callAPI('api/transaction/', 'GET', null, {
        authorization: access_jwt_token,
      });
      let data = getTransactions.data.message.map((trans) =>
        formatTransaction(trans),
      );
      setData(data);
    };
    getData();
  ...
  return (
    <div>
      <OrderTable tableData={data} tableHeader={header} />
    </div>
  );
}
